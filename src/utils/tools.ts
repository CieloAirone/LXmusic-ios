import { NativeModules, Platform, ToastAndroid, BackHandler, Linking, Dimensions, Alert, Appearance, PermissionsAndroid, AppState, StyleSheet, type ScaledSize } from 'react-native'
...
export const checkStoragePermissions = async() => !isAndroid || PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE)
...
export const toast = (message: string, duration: 'long' | 'short' = 'short', position: 'top' | 'center' | 'bottom' = 'bottom') => {
  if (!isAndroid) {
    NativeModules.LXDevice.toast(message, duration === 'long' ? 3.5 : 2, position)
    return
  }
  let _duration
  switch (duration) {
