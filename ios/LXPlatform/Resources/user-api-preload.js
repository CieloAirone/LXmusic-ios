...
globalThis.lx_setup = (key, id, name, description, version, author, homepage, rawScript) => {
  ...
  const nativeFuncNames = [
    ...
    '__lx_native_call__utils_buf2str',
  ...
  // 将字节数组解码为字符串（UTF-8）
  // iOS port: native UTF-8 conversion handles non-BMP characters and binary data.
  function bytesToString(bytes) {
    return nativeFuncs.utils_buf2str(JSON.stringify(Array.from(bytes)))
  }
  function stringToBytes(text) {
    return JSON.parse(nativeFuncs.utils_b642buf(nativeFuncs.utils_str2b64(text)))
  }
